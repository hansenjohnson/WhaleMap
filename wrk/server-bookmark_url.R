...
function(input, output, session){
  ...
  ydays <- reactive({
    if (input$go == 0){ 
      # choose date from URL or default (only on startup)
      ...
      if (!is.null(query[['from']])) {
        ...
        # determine date limits from url
        d0 = as.Date(query[['from']])
        d1 = as.Date(query[['to']])
        
        # update date slider
        updateSliderInput(session, "date", value = c(d0, d1))
